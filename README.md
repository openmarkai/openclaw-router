...
The plugin uses a **two-turn hybrid architecture**: Turn 1 classifies and routes, Turn 2 gets the response from the optimal model. OpenClaw handles all authentication and API formatting — the plugin never touches your API keys.
...
User sends follow-up message
    |
    v
OpenClaw routes to the optimal model (written in Turn 1)
  - Full session context, system prompt, conversation history
  - Authentication and streaming handled by OpenClaw
    |
    v
User receives response from the best model for their task
    |
    v
Auto-restore: openmark/auto is set back as default for next routing
```

**Zero API key access.** Classification goes through the OpenClaw gateway. Model switching writes to OpenClaw's config. The plugin never makes direct calls to any provider API.
...
|-------|---------|-------------|
| `classifier_model` | `""` (your default model) | Override: model for classification calls. Leave empty to use your current default model. |
| `no_route_passthrough` | `""` (your default model) | Override: model for unrouted messages. Leave empty to use your current default model. |
...
By default, the router uses **your existing default model** for both classification and passthrough. The plugin captures your default model on startup (before setting `openmark/auto`), so it works with any provider you have configured — no need to set up a specific model.
...
- **Uses your default model**: Classification and passthrough use whatever model you already have configured. No dependency on any specific provider.
