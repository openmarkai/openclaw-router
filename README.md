...
The routed model genuinely answers. The router performs an isolated classification step first, then OpenClaw produces the real reply with the routed model stack in the same user-visible response.
...
The plugin uses an **internal two-phase architecture**: Phase 1 classifies and routes, Phase 2 generates the real reply from the optimal model. To the user, this should still appear as a single reply. OpenClaw handles all authentication and API formatting — the plugin never touches your API keys.
